...
export class MongoDB extends Crud {
    ...
    async connect() {
        ...
        console.log(`mongoose conectado en ${uri}`)
            ...
            // this.modelo.find({}).then(productos => console.log('productoss', productos)).catch(err => console.log('hubo error :(:(:('))
        return
    }
    ...
    obtenerUno(id, callback) {
        this.modelo.findById(id)
            .then(producto => callback(null, producto))
            .catch(err => callback(err, null));
    }

    agregarUno(item, callback) {
        this.modelo.create(item)
            .then(producto => callback(null, producto))
            .catch(err => callback(err, null));
    }

    actualizarItem(id, item, callback) {
        this.modelo.findByIdAndUpdate(id, item)
            .then(producto => callback(null, producto))
            .catch(err => callback(err, null));
    }

    borrarItem(id, callback) {
        this.modelo.findByIdAndDelete(id)
            .then(producto => callback(null, producto))
            .catch(err => callback(err, null));
    }
